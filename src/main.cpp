...
#if defined(NDEBUG)
# error "Reaiscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x282d061f01ba16de6ffff881e739aeb4f4e77997962d9696798bf6de8709ce16");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Reaiscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Reaiscoin Signed Message:\n";
...
{
    // Reaiscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Reaiscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Reaiscoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Reaiscoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Reaiscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Reaiscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x282d061f01ba16de6ffff881e739aeb4f4e77997962d9696798bf6de8709ce16");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 01/Apr/2018 Syria war: 'Deal struck' to take critically injured out of Douma";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("526984710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522553121;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2778148;
        ...
        assert(block.hashMerkleRoot == uint256("0x92cc6f12703167f143a42175823fd53116542b47fda67d6885abd97e435ff628"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Reaiscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ReaiscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ReaiscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ReaiscoinMiner(CWallet *pwallet)
{
    printf("ReaiscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reaiscoin-miner");
    ...
    try { loop {
        ...
        printf("Running ReaiscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ReaiscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ReaiscoinMiner, pwallet));
}
